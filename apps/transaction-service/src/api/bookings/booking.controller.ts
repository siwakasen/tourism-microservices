...
import {
  AssignEmployeeDto,
  BookingRegisterReqDto,
  BookingRegisterResDto,
  BookingReqDto,
  BookingResDto,
  FinishBookingDto,
  PaginationDto,
...
export class BookingController {
  ...
  private bookingValidation(payload: BookingReqDto) {
    if (!payload.package_id && !payload.car_id) {
      throw new HttpException(
        'Package or car is required',
        HttpStatus.BAD_REQUEST
      );
    }
    if (payload.package_id && payload.car_id) {
      throw new HttpException(
        'Cannot booking both package and car at the same time',
        HttpStatus.BAD_REQUEST
      );
    }
  ...
  @Post('/and-register')
  async bookingWithRegister(
    @Body() body: BookingRegisterReqDto
  ): Promise<BookingRegisterResDto> {
    this.bookingValidation(body);

    const { token, customer_id } =
      await this.bookingService.registerCustomerGrpc(body);
