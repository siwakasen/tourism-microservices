...
export class RefundService {
  ...
  public async createRefund(
    ...
    const refund = queryRunner.manager.create(Refunds, {
      ...
      method:
        payment[0].payment_method === PaymentMethod.PAYPAL
          ? RefundMethod.PAYPAL
  ...
  public async getRefundById(customer_id: number, refund_id: number) {
    try {
      const queryBuilder = this.dataSource.manager
        .createQueryBuilder(Refunds, 'refunds')
  ...
  public async saveForm(
    ...
    try {
      const refund = await queryRunner.manager.findOne(Refunds, {
        where: {
          id: refund_id,
          booking: { customer_id },
          status: RefundStatus.WAITING_FORM,
  ...
  public async completeRefund(refund_id: number) {
    ...
    try {
      const refund = await queryRunner.manager.findOne(Refunds, {
        where: { id: refund_id, status: RefundStatus.PROCESSING },
      });
      if (!refund) {
        throw new HttpException('Refund not found', HttpStatus.NOT_FOUND);
      }
